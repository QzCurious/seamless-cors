...
A new Gateway Owner has a synchronous prepare phase before it is discoverable: generate token, construct facade, construct router, and bind the HTTP router listener. Activation then starts HTTP serving, writes the Gateway State Cache, starts the lease watcher, optionally runs direct `start` activation, and enters the owner run loop. Direct `start` may therefore be owner-visible while CA trust approval is pending, but Gateway Runtime activation, managed PAC installation, and Start Guidance still wait for CA Ensure to finish. If cache write fails after serving starts, the server shuts down and the process exits without becoming owner.
